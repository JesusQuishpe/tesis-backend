<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateCoprologiaTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('coprologia', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger("id_cita");
            $table->foreign("id_cita")->references("id")->on("citas");
            $table->unsignedBigInteger("id_doc")->nullable()->default(null);
            $table->foreign("id_doc")->references("id")->on("doctores");
            $table->unsignedBigInteger("id_tipo");
            $table->foreign("id_tipo")->references("id")->on("tipo_examen");
            $table->string("consistencia",100)->default('')->default('');
            $table->string("moco",45)->default('');
            $table->string("sangre",45)->default('');
            $table->float("ph")->default(0);
            $table->string("azucares_reductores",45)->default('');
            $table->string("levadura_y_micelos",45)->default('');
            $table->string("gram",100)->default('');
            $table->string("leucocitos",45)->default('');
            $table->string("polimorfonucleares",100)->default('');
            $table->string("mononucleares",100)->default('');
            $table->string("protozoarios",100)->default('');
            $table->string("helmintos",100)->default('');
            $table->string("esteatorrea",100)->default('');
            $table->string("observaciones",200)->default('');
            $table->boolean('atendido')->default(false);
            $table->boolean('eliminado')->default(false);
            $table->timestamps();

        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('coprologia');
    }
}
